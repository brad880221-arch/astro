/**
 * 한국 B2B PC 조달 비용 & 최적 타이밍 시뮬레이터 (고도화 버전)
 * Interactive Procurement & Cost Forecasting Engine with Presets & Comparison
 */
...
const SemiSimulator = {
    ...
    presetButtons.forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        if (qty && qtySlider) {
          ...
          if (qtyValueDisplay) qtyValueDisplay.textContent = qty.toLocaleString() + ' 대';
          this.calculate();
        }
      });
    });

    // 4. 반도체 가격 시나리오 라디오 버튼
  ...
  calculate() {
    ...
    // 1) 반도체 가격 변동률 반영
    const semiFactor = scenarioData.factor;
    let adjustedASP = baseASP * (1 + semiFactor);

    // 2) AI PC NPU 탑재 추가 프리미엄 반영 (가중치)
    ...
    const baseTotalBudget = baseASP * this.state.quantity;
    const totalBudget = finalASP * this.state.quantity;

    // 기준 가격 대비 변동율
    const totalVarianceRate = ((finalASP - baseASP) / baseASP) * 100;
    const totalDiffAmount = totalBudget - baseTotalBudget;
  ...
  updateUI(results) {
    const { baseASP, finalASP, baseTotalBudget, totalBudget, totalVarianceRate, totalDiffAmount, scenarioData } = results;
    ...
    // 비교 카드 요소
    const compareBaseDisplay = document.getElementById('simCompareBase');
    const compareFinalDisplay = document.getElementById('simCompareFinal');
    ...
    if (compareBaseDisplay) compareBaseDisplay.textContent = (baseTotalBudget / 100000000).toFixed(2) + '억 원';
    if (compareFinalDisplay) compareFinalDisplay.textContent = (totalBudget / 100000000).toFixed(2) + '억 원';
    ...
    if (this.state.scenario === 'surge' || this.state.scenario === 'slight_up') {
      ...
      recText = `현재 선택하신 반도체 가격 상승 국면에서는 PC 제조사의 4분기 공급단가 인상이 예정되어 있습니다. 현재 시점에서 계약을 확정할 경우 예상 예산 대비 약 ${Math.abs(Math.round(totalDiffAmount / 10000)).toLocaleString()}만 원의 원가 상승 리스크를 선제적으로 회피할 수 있습니다.`;
    } else if (this.state.scenario === 'crash') {
      recTitle = '⏳ [분할 조달 권고] 10월 초 메모리 가격 반영 시점까지 관망 유리';
      recText = `반도체 재고 증가 및 가격 하락세가 관측되므로 즉각적인 대량 발주보다는 긴급분(30%)만 우선 조달하고, 잔여 물량은 제조사의 프로모션 단가 인하가 고시되는 10월 중순 이후 계약하는 것이 약 ${Math.abs(Math.round(totalDiffAmount / 10000)).toLocaleString()}만 원의 예산 절감에 유리합니다.`;
    } else {
